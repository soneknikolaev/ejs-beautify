...
describe('Beatify template without custom delimiters', () => {
    ...
    it('Complex EJS template', () => {
        const template = beautify('<div><h2>Hello <%= user.name %>!</h2><%if(links&&links.length>0){%><h3>Also, you received a link(s):</h3><%for(var i=0;i<links.length;i++){%><h3><%=links[i]%></h3><%}%><%}%></div>');
